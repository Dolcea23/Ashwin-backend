...
export default function Profile() {
  ...
  const saveProfile = async () => {
    const profile = { name, age, gender, height, weight, sleepGoal, notifications, autoConnect };
...
const handleLogout = () => {
  Alert.alert("Log Out", "Are you sure you want to log out?", [
    { text: "Cancel", style: "cancel" },
    {
      ...
      onPress: async () => {
        try {
          // 🧽 Clear all relevant flags and stored user data
          ...
          router.replace("/signin"); // or "/(auth)/signin" if your folder uses groups
        } catch (e) {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.container}>
        <Text style={styles.header}>{name ? `${name}'s Profile` : "Your Profile"}</Text>
        <Text style={styles.subheader}>Personalized for balance, growth, and wellness.</Text>
        ...
        <View style={styles.card}>
          ...
          <Input label="Height (in)" value={height} onChangeText={setHeight} keyboardType="numeric" />
          <Input label="Weight (lbs)" value={weight} onChangeText={setWeight} keyboardType="numeric" />
          <Input label="Sleep Goal (hours)" value={sleepGoal} onChangeText={setSleepGoal} keyboardType="numeric" />
        </View>
        ...
        <View style={styles.card}>
          ...
          <SwitchRow label="Enable Notifications" value={notifications} onValueChange={setNotifications} />
          <SwitchRow label="Auto Bluetooth Reconnect" value={autoConnect} onValueChange={setAutoConnect} />
        </View>
        ...
        <View style={styles.card}>
          ...
          {trialActive ? (
            <>
              ...
              <Text style={styles.priceText}>After trial: $9.99/month or $100/year</Text>
            </>
          ) : (
            <>
              <Text style={styles.trialText}>
                Enjoy <Text style={{ fontWeight: "700" }}>7-Day Free Trial</Text> — deeper AI insights & feedback loops.
              </Text>
        ...
        <View style={{ marginTop: 30 }}>
          <Text style={styles.footerLine}>Designed for Reflection, not Diagnosis.</Text>
...
const Input = ({ label, ...props }: any) => (
  <View style={{ marginBottom: 10 }}>
    ...
    <TextInput {...props} style={styles.input} placeholder={label} placeholderTextColor="#aaa" />
  </View>
);
...
const styles = StyleSheet.create({
  ...
  header: { fontSize: 24, fontWeight: "700", textAlign: "center", color: "#0D47A1" },
  subheader: { textAlign: "center", color: "#555", fontSize: 13, marginBottom: 25 },
  sectionTitle: { fontSize: 16, fontWeight: "700", color: "#0D47A1", marginBottom: 10 },
  card: {
    backgroundColor: "#fff",
    borderRadius: 14,
    padding: 16,
  ...
  trialBtn: { backgroundColor: "#1877F2", borderRadius: 10, paddingVertical: 10, alignItems: "center" },
  trialBtnText: { color: "#fff", fontWeight: "700" },
  priceText: { textAlign: "center", fontSize: 12, color: "#666", marginTop: 8 },
  logoutBtn: { backgroundColor: "#E53935", borderRadius: 10, paddingVertical: 12, alignItems: "center", marginTop: 10 },
  logoutText: { color: "#fff", fontWeight: "700" },
  footerLine: { textAlign: "center", color: "#777", fontSize: 12 },
  footerBrand: { textAlign: "center", color: "#333", fontSize: 12, marginTop: 2 },
});
