...
def board(req: Request):
    ...
    if not users:
        users = [(1, "User 1")]   # Fake placeholder user
        selected = 1
    else:
        q_user = req.query_params.get("user")
        selected = int(q_user) if q_user else users[0][0]
    ...
    # Selected time range
    range_key = req.query_params.get("range") or "session"
...
if not rows:
    ...
    for (eeg, ecg, temp, light, noise, ts) in rows:
        ...
        n = len(harmonies)
        mid = max(1, n // 2)          # before always has at least 1 point
        before = harmonies[:mid]
        after = harmonies[mid:]
        ...
        # ---------- Zone Inference (inferred field region dominance) ----------
        zone = infer_zone_from_window(eeg_vals, ecg_vals, temp_vals)
    ...
    <small class="text-muted">
      Confidence: <span id="kpiZoneConf">{zone["confidence"]}</span>
        ...
        # ---------- Chart arrays (downsample + smooth for graphs) ----------
        chart_times, chart_harmonies, chart_eeg, chart_ecg = downsample_and_smooth(
            times_raw, harmonies, eeg_vals, ecg_vals, step=5, window=20
        )
